package com.amok.ringprogressbar;
...
public class RingProgressBar extends View {
  ...
  public RingProgressBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    showText = mTypedArray.getBoolean(R.styleable.RingProgressBar_showText, true);
    style = mTypedArray.getInt(R.styleable.RingProgressBar_style, 0);
    padding = dp2px( mTypedArray.getDimension(R.styleable.RingProgressBar_fillPadding, 5));
    //初始化padding值 默认设置为5
    mTypedArray.recycle();
  }
  ...
  private void drawTextContent(Canvas canvas) {
    ...
    if (showText && percent != 0 && style == STROKE) {
      if(Tip != null){
        textWidth = paint.measureText(Tip);
        canvas.drawText(Tip, centre - textWidth / 2, centre + textSize / 2, paint);
      }else {
